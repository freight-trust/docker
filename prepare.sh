...
## Install RVM.
export HOME=/root
run gpg --import freight-trust-pubkey.asc
run bash rvm-install.sh stable
...
run install -o root system-rvm-exec.sh /usr/bin/rvm-exec
...
run cp dpkg-control ruby-fake/DEBIAN/control
